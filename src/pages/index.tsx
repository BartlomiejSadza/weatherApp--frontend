...
export default function Home({
	...
	const [updatedWeatherData, setUpdatedWeatherData] =
		useState<WeatherData[]>(weatherData);
	const [updatedWeeklyData, setUpdatedWeeklyData] =
		useState<WeeklyData>(weeklyData);
	...
	useEffect(() => {
		if (location) {
			const fetchWeatherData = async () => {
				try {
					const res = await fetch(
						`/api/endpoint1?lat=${location.lat}&lon=${location.lon}`
					);
	...
	useEffect(() => {
		if (location) {
			const fetchWeeklyData = async () => {
				try {
					const res = await fetch(
						`/api/endpoint2?lat=${location.lat}&lon=${location.lon}`
					);
					if (!res.ok) throw new Error(`HTTP error! status: ${res.status}`);
			...
			<div
				...
				<main className={styles.main}>
					...
					<footer className={styles.weeklySummary}>
						<Parametr
							parametr={updatedWeeklyData.averagePressure}
							ikonka={ikonki.pressure}
							tytuł='Ciśnienie'
						/>
						<Parametr
							parametr={updatedWeeklyData.averageSunshineDuration}
							ikonka={ikonki.sunshine}
							tytuł='Nasłonecznienie'
						/>
						<Parametr
							parametr={updatedWeeklyData.maxTemperature}
							ikonka={ikonki.tempHigh}
							tytuł='Temp. maksymalna'
						/>
						<Parametr
							parametr={updatedWeeklyData.minTemperature}
							ikonka={ikonki.tempLow}
							tytuł='Temp. minimalna'
						/>
						<Parametr
							parametr={updatedWeeklyData.precipitationDays}
							ikonka={ikonki.rain}
							tytuł='Dni opadów'
						/>
						<Parametr
							parametr={updatedWeeklyData.windAverage}
							ikonka={ikonki.wind}
							tytuł='Prędkość wiatru'
						/>
					</footer>
				</main>
			</div>
